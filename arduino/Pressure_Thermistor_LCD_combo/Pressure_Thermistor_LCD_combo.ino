...
float Vin = 5.;           // Defining the input volate as 5V
...
float tempVin=5.;        // Defining the input volate as 5V

//Taking a measurement
void setup()
{
  firstThermistor.begin(5);
...
void loop()
{
  ...
  pressureVout = (analogRead(PressurePin)/1024.)*Vin;    // read the input pin (scaled to the volatge input)
  pressure=((pressureVout/5.)+.04)/.004;                       // scaling in kPa from data sheet
